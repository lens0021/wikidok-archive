...
describe('reversedIter', () => {
  ...
  for (const i of [...Array(24).keys()].map(
    (el) => el + 1,
  )) {
  ...
  it.each([
    ...
    expect(Module.reversedIter(revisions)).toStrictEqual(
      expected,
    )
    msg
  })
  ...
  it.each([
    ...
    },
    {
      ...
      revs: {
        ...
        '1': {
          wikiTitle: 'dummy',
          timestamp: '2009-05-28T07:46:54Z',
        },
      },
      expected: {
        '3': {
    ...
    const actual = Module.fillMissingRevisions(
      revs,
      dummySiteInfo,
    )
...
describe('findLatestRevisionCount', () => {
  ...
  it.each`
    ...
    ${{ '2': rev, '1': rev }} | ${2}
  `('$rev', ({ rev, expected }) => {
    expect(Module.findLatestRevisionCount(rev)).toBe(
      expected,
    )
  })
})
...
describe('escapeSpecialCharacters', () => {
  ...
  `('$title', ({ title, expected }) => {
    expect(Module.escapeSpecialCharacters(title)).toBe(
      expected,
    )
  })
})
...
it.each([
  ...
  expect(replaceHtmlTags.replaceHtmlTags(html)).toBe(
    expected,
  )
})
